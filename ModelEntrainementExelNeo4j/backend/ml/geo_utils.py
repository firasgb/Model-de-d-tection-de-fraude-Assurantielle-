"""
Gocodage hybride pour la Tunisie  VERSION 1.0

Convertit des adresses textuelles en coordonnes GPS (latitude/longitude) en
trois tapes successives, par ordre de priorit :

  1. Lookup dans un gazetteer offline (gouvernorats + villes/dlgations
     tunisiennes connues). Rapide, sans rseau, gratuit.
  2. Cache disque persistant (geocode_cache.json) pour mmoriser les rsultats
     dj rsolus (offline ou via API) et viter de relancer une requte.
  3. Fallback API Nominatim (OpenStreetMap)  facultatif, activable via
     `enable_api=True`. Respecte les CGU (1 req/s + User-Agent identifi).
...
~120 dlgations / villes principales. Les correspondances sont fuzzy
(suppression des accents, espaces multiples, ponctuation) pour rsister
...
# 
# Gazetteer offline : gouvernorats + grandes villes / dlgations tunisiennes
# Coordonnes approximatives (centre ville). Les distances Haversine restent
# fiables  l'chelle d'un sinistre (rsolution  quelques km).
# 
...
TUNISIA_GAZETTEER: Dict[str, Tuple[float, float]] = {
    ...
    #  Sud (Gabs, Tataouine, Kbili, Tozeur) 
    ...
    "moulars":          (34.4825, 8.2617),
    "moulares":          (34.4825, 8.2617),
    #  Nord-Ouest (Bja, Jendouba, Kef, Siliana, Bizerte) 
    ...
    #  Mots-cls frontires (utiliss aussi par sinistre_frontiere) 
...
def _strip_accents(s: str) -> str:
    """Supprime les accents (e, a, etc.)  essentiel pour le fuzzy match."""
...
# 
# Gazetteer pr-normalis (calcul une fois au chargement du module)
# 
...
class TunisiaGeocoder:
    """Gocodeur hybride offline + cache + Nominatim (optionnel).

    Args:
        cache_path: chemin du cache JSON disque (lectures/critures persistantes).
            Si None, le cache reste uniquement en mmoire.
        enable_api: si True, appelle Nominatim en fallback quand le gazetteer
            ne trouve aucune correspondance. Dsactiv par dfaut (offline only).
        api_min_interval: dlai minimal entre 2 requtes API (CGU Nominatim  1s).
    ...
    def __init__(
        ...
        if cache_path and os.path.exists(cache_path):
            try:
                ...
                print(f"    Geocoder: cache charg ({len(self._memory_cache)} entres) depuis {cache_path}")
            except Exception as e:
                print(f"   [WARN]  Geocoder: cache illisible ({e}), dmarrage  vide")
    ...
    def _lookup_offline(self, normalized: str) -> Optional[Tuple[float, float]]:
        ...
        puis sous-chane  la plus longue gagne pour limiter les faux positifs)."""
        if not normalized:
            return None
    ...
    def _lookup_api(self, raw_address: str) -> Optional[Tuple[float, float]]:
        """Appelle Nominatim (OpenStreetMap) pour gocoder une adresse libre.
        Retourne None en cas d'chec, de timeout ou de rsultat hors Tunisie."""
        try:
        ...
        try:
            ...
            # Borne de scurit : Tunisie  [30N, 38N]  [7E, 12E]
    ...
    def geocode(self, address: Any) -> Tuple[Optional[float], Optional[float]]:
        """Gocode une adresse unique.

        Returns:
            (latitude, longitude) ou (None, None) si non rsolue.
        """
        ...
        # 4) Aucun rsultat  on mmorise (None, None) pour ne pas re-tenter
    ...
    def geocode_series(self, series: pd.Series) -> Tuple[pd.Series, pd.Series]:
        """Gocode une colonne pandas entire. Retourne (lat_series, lon_series)
        alignes sur l'index de l'entre."""
    ...
    def save_cache(self) -> None:
        """Sauvegarde le cache mmoire sur disque (JSON). No-op si pas de path."""
        ...
        try:
            ...
            print(f"    Geocoder: cache sauvegard ({len(serializable)} entres)  {self.cache_path}")
        except Exception as e:
            print(f"   [WARN]  Geocoder: chec sauvegarde cache ({e})")
...
# 
# Distance Haversine (kilomtres)  expose ici pour viter une dpendance
